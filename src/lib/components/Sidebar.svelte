...
{#snippet navIcon(href: string)}
	...
	>
		{#if href === '/agenda'}
			<path d="M8 6h13" /><path d="M8 12h13" /><path d="M8 18h13" /><path d="M3.5 6h.01" /><path
				d="M3.5 12h.01"
			/><path d="M3.5 18h.01" />
		...
		{:else if href === '/timetable'}
			<rect x="3" y="4.5" width="18" height="16.5" rx="3" /><path d="M3 10h18" /><path
				d="M9 10v11"
			/><path d="M15 10v11" />
		{:else}
...
<aside class="flex w-64 shrink-0 flex-col border-r border-line bg-[#FAF6F7] p-4 max-md:w-[200px]">
	...
	<div class="mt-auto flex flex-col gap-0.5 border-t border-line pt-3">
		<a
			...
			style:color={active('/settings') ? 'var(--color-pink-dk)' : 'var(--color-grey-1)'}
		>
			<svg
				width="19"
				height="19"
				...
				stroke-linecap="round"
				stroke-linejoin="round"
				><path d="M4 8h16" /><circle cx="9" cy="8" r="2.4" /><path d="M4 16h16" /><circle
					cx="15"
					cy="16"
					r="2.4"
				/></svg
		...
		<button
			type="button"
			onclick={onsignout}
			class={`${item} text-grey-3 hover:bg-pink-50 hover:text-pink`}
		>
			<svg
				width="19"
				height="19"
